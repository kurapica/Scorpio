        ...
        if type(ui.HasScript) == "function" then
            for _, evt in Enum.GetEnumValues(ScriptsType) do
                if ui:HasScript(evt) and not Class.GetFeature(super, evt, true) then
                    __EventChangeHandler__(function (delegate, self, name, init)
                        return init and self:HasScript(name) and self:HookScript(name, function(_, ...) return delegate(self, ...) end)
                    end)
                    _ENV.event(evt)
                end
            end
