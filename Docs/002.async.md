...
So we only need use `__Async__()` attribute on the function, so it'll be processed in
the Lua coroutine, that means we can use APIs like `Delay` to yield the functio process
...
Although the Scorpio recommend use the Asynchronous functions, we still can pass function to
...
You also can use the outside the Scoprio addon modules, like call `Scorpio.Delay`. Please keep 
in mind the callback function will also be processed in a coroutine.
...
why we normally use the callback not the coroutine(also the coroutine is a little hard for
common authors).
