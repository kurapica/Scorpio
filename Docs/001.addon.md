...
There are special events for unit, The system also provides another attribute to handle the unit events:
...
-- Or you can provide the event by the function name
__UnitEvent__ "player"
function UNIT_HEALTH()
end
```

So those functions will be called when the player's health changed.
