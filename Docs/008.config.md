...
```lua
__Config__(_Config.Panel, "alpha", RangeValue[{0, 1}], 1):DisableUI()
function SetAlpha(alpha)
	-- the current value can be fetched from the subject or the parameter
	assert(val == _Config.Panel.alpha:GetValue())
end
...
```lua
__Config__(_Config.Panel, "alpha", RangeValue[{0, 1}], 1):DisableQuickApply()
function SetAlpha(alpha)
	-- the current value can be fetched from the subject or the parameter
	assert(val == _Config.Panel.alpha:GetValue())
end
