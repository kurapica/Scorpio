...
function OnLoad()
	_Addon:SetSavedVariable("TestAddonSave")          -- Bind the global saved variable
		:SetCharSavedVariable("TestAddonCharSave")    -- Bind the char saved variable
...
function SetAlpha(alpha)
	...
	assert(val == _Config.Panel.alpha:GetValue())
end
-- Update the UIParent's alpha by the node field
Style[UIParent].alpha = _Config.Panel.alpha
```
...
__Config__(_Config, "actionbars", {
	...
	{   -- The table means a new member definition
		...
		row = { type = RangeValue[{1, 12, 1}], default = 1 },
		col = { type = RangeValue[{1, 12, 1}], default = 12 },
...
![Test Addon Dictionary Type](./pics/config_dict.gif)
...
class "ActionTypeViewer" (function(_ENV)
	...
	local function OnViewerHyperlinkClick(self, data)
		-- The IConfigSubjectHandler interface provide a method
		-- to write back field value
		self:SetConfigSubjectValue(data)
	end
    ...
    function SetConfigSubject(self, configSubject)
    	...
    	-- Generate the html
    	self:SetText(TEMPLATE{ items = items })
    end

	-- Define the constructor, used to bind ui event handlers
	function __ctor(self)
		self.OnViewerHyperlinkClick = self.OnViewerHyperlinkClick + OnViewerHyperlinkClick
	end
end)
...
Style.UpdateSkin("Default", {
	[ArrayStructTypeViewer] = {
        minResize = Size(100, 400), -- set the min height
		marginRight = 0,  -- set the margin, so viewer will take the full width
	}
})
```
