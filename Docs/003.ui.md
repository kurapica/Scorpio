...
1. Please Install the [Cube](), you can use
    ...
    to open a in-game browser to show the featuers of those UI classes. Also you
    can bind keys to open them in the key-binding frame.
...
Each ui element has its own object type, like **Button** which provide the mouse click feature.
Normally blizzard will create ui Elements based on templates which defined the same settings for
a group of ui elements, like the location, the size, the child elements and etc, take an example:
...
So the template's object type is **EditBox** which can be used to receive the user's text inputs.
The `vitual="true"` means it's also a template, and the template name is **InputBoxTemplate**, a
template can have another template as its super, so it don't need to defined the same things that
its super template defined.

In the xml, the **InputBoxTemplate** has two parts declared:

1. Layers - There are many draw layers to display text, pics, so we can put some pic as the background,
    some text above others. In the example, the template only used the **BACKGROUND** layer, where
    it use three **Texture** elements, the **Texture** are used to display pics.

2. FontString - The **FontString** is also an object type used to display text, but here it also means
    a property of the **EditBox**(The Editbox has methods `SetFontString` & `GetFontString` to access it).
    so it's a child element of the **EditBox**, and it use the **ChatFontNormal** as super templatem.


The other tags like **atlas**, **Size**, **Anchor** are all properties to the elments.


You don't need to spend too much time on the xml, the Scorpio has provided a new UI style system to
simple it, and with the [Cube](), we can easily browse the features of the ui system.


The object types of the UI elements can be splitted into four categories:

1. The elements that used to display text: FontString.

2. The elements that used to display pictures: Texture, MaskTexture, Line.

3. The elements that used to provide the user interaction: Frame, Button, CheckButton, ColorSelect, Cooldown, EditBox, MessageFrame, ScrollFrame, SimpleHTML, Slider, StatusBar, Model, ModelScene, PlayerModel, CinematicModel, DressUpModel, TabardModel and etc.

4. The elements that used to create animations: AnimationGroup, Animation, Alpha, Path, ControlPoint, Rotation, Scale, LineScale, Translation, LineTranslation.

Some object type like **GameTooltip** is removed from the list since they are versy special to be used, and we don't really need to create them.
